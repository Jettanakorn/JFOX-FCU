...
| HWR-ENV-006 | Every power input shall carry transient suppression. | DO-160G §17 voltage spike. D4, 6 V standoff on +5V_CARRIER. The brick and servo rails left the card with the power tree; USB clamping is U40's job. The backplane feeds are NOT yet suppressed - see docs/VOLTAGE_ANALYSIS.md — above the 5.5 V a valid supply reaches, below anything the LTC4417 or the buck will tolerate. Unidirectional: reverse battery is the LTC4417's job. | `check_fmu_objective :: power inputs have transient suppression` | V | X |
